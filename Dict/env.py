...
class MetaGames:
    ...
    def select_action(self, state):
        ...
        if np.random.random() < self.epsilon:
            ...
            seed = torch.randint(0,2, (1, )).to(device)
            ...
            poss_max = torch.argwhere(self.innerq[1, state, :] == torch.max(self.innerq[1, state, :])).to(device) 
            action = self.action_unmapping(random.choice(poss_max))   #find maximum table from action dimension
