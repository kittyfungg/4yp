...
def mp_one_iteration_batched(bs):
    ...
    payout_mat_1 = torch.Tensor([[0, 1], [1, 0]]).to(device)    #changed -1 entries to 0 since cant negative-index
    payout_mat_2 = payout_mat_1.T
    rew1 = torch.empty(bs).to(device)
    rew2 = torch.empty(bs).to(device)
    ...
    def Reward(action):
        for i in range(bs):
            x = torch.stack((action[0,i], 1-action[0,i]), dim=0)    #[our agent, updated action, batch]
            y = torch.stack((action[1,i], 1-action[1,i]), dim=0)
            rew1[i] = torch.matmul(torch.matmul(x, payout_mat_1), y.unsqueeze(-1)).squeeze(-1).detach().clone()
            rew2[i] = torch.matmul(torch.matmul(x, payout_mat_2), y.unsqueeze(-1)).squeeze(-1).detach().clone()
...
class MetaGames:
    def __init__(self, b, opponent="NL", game="IPD"):
        ...
        d, self.game_batched = mp_one_iteration_batched(b)
        self.std = 1
        self.lr = 1
        self.d = d[0]    
